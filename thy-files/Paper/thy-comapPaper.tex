...
\definecolor{ccr}{RGB}{0,128,172}

\usepackage{hyperref}
\hypersetup{
	colorlinks=true,     % 启用链接颜色
	linkcolor=ccr,       % 设置标号引用和内部链接的颜色
	citecolor=ccr,       % 设置文献引用的颜色
	pdfborder={0 0 0},   % 移除链接的边框
	linktoc=none,        % 不改变目录链接的颜色
	frenchlinks=true     % 禁用页眉链接的颜色
}
...
\begin{wrapfigure}{r}{0.5\linewidth}  % 'r' for right, '0.7\linewidth' for the width of the figure
	\centering
...
To seek evidence of the existence of the great coach effect, we employ the Difference-in-Differences (DiD) \cite{becketti2009big} model to examine its impact.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{C:/Users/Lenovo/Downloads/nannv}
